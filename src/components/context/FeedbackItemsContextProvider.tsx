...
export default function FeedbackItemsContextProvider({children}:FeedbackItemsContextProviderProps) {
  ...
  const companyList = useMemo(() => feedbackItems.map((item) => (item.company))
    .filter((compnay,index,array) => {
      return array.indexOf(compnay) === index;
    }),
    [feedbackItems]);
  ...
  const filteredFeedbackItems = useMemo(() => selectedCompany? feedbackItems
    .filter(feedbackItem => feedbackItem.company === selectedCompany)
    : feedbackItems,
